...
% Bandpass filter for initial visualization
[b_bp, a_bp] = butter(4, [0.5 40]/(fs/2), 'bandpass');
eeg_filtered_temp = filtfilt(b_bp, a_bp, raw_eeg);
...
xlabel('Time (s)');
ylabel('Theta Power (0.5-4 Hz)');
title('Time-Resolved Delta Power');
grid on;
hold on;
...
legend('Alpha Power', 'Baseline','Eyes Closed & Rest','Thinking & Eyes Open');
hold off;
