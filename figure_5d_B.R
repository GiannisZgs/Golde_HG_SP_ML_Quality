    ...
    if (nrow(subset_annotations) > 0) {
      ...
      if (all(!is.na(subset_annotations$p_wave_feature))&& show_labels) {
        ...
        p <- p + 
            ...
            fontface = "bold",
            size = 3,
            box.padding = 0.8,          # Increased box padding
            point.padding = 0.5,        # Increased point padding
            force = 10,                 # Increased repulsion force
            max.overlaps = 20,          # Increased max overlaps
            min.segment.length = 0,   
            label.padding = unit(0.2, "lines"),  # Increased label padding
            label.r = unit(0.2, "lines"),        # Increased corner radius
            direction = "y",         
            hjust = 1,
            vjust = 0.5,
            nudge_x = -0.05,             # Increased nudge left
            segment.color = "black",  
            segment.size = 0.5,
            xlim = c(min(subset_data$Time) + 0.05, max(subset_data$Time) - 0.05), # Keep within plot bounds
            ylim = c(min(subset_data$Amplitude) * 1.1, max(subset_data$Amplitude) * 0.9), # Keep within plot bounds
            inherit.aes = FALSE
            )
        }
