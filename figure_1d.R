...
# Extract the k-means cluster mapped waveforms for each participant, channel, and sensor
for (participant in selected_participants) {
  for (channel in selected_channels) {

    if (participant %in% names(data$centroids$AgCl[[channel]])) {
      agcl_waveform <- data$centroids$AgCl[[channel]][[participant]]$kmeans_cluster_mapped_waveform
      
      if (participant %in% names(data$features)) {
        p_wave_amp <- data$features[[participant]]$AgCl[[channel]]$p_wave_amp
        peak_to_peak <- data$features[[participant]]$AgCl[[channel]]$peak_to_peak
        t_wave_amp <- data$features[[participant]]$AgCl[[channel]]$t_wave_amp
    ...
    # Check if the data exists for HG sensor
    if (participant %in% names(data$centroids$HG[[channel]])) {
      hg_waveform <- data$centroids$HG[[channel]][[participant]]$kmeans_cluster_mapped_waveform
            
      # Extract features for PPHG if available
      if (participant %in% names(data$features)) {
        p_wave_amp <- data$features[[participant]]$HG[[channel]]$p_wave_amp
        peak_to_peak <- data$features[[participant]]$HG[[channel]]$peak_to_peak
        t_wave_amp <- data$features[[participant]]$HG[[channel]]$t_wave_amp
...
for (participant in selected_participants) {
  for (channel in selected_channels) {
    for (sensor in c("AgCl", "PPHG")) {
      subset_data <- waveform_data[waveform_data$Participant == participant & 
      ...
      if (nrow(subset_data) > 0) {
        ...
        feature_row <- features_data[features_data$Participant == participant & 
                                     features_data$Channel == channel &
...
# Create a faceted plot of the waveforms with annotations and shaded areas
plot_list <- list()

for (participant in selected_participants) {
  for (channel in selected_channels) {
    # Get data for this participant and channel
    subset_data <- waveform_data[waveform_data$Participant == participant & 
                               waveform_data$Channel == channel, ]
    
    subset_annotations <- annotations[annotations$Participant == participant & 
    ...
    if (nrow(subset_data) > 0) {
      ...
      # Add shaded areas properly - create separate dataframes for each sensor
      ...
      if (nrow(subset_annotations) > 0) {
        ...
        if (all(!is.na(subset_annotations$p_wave_feature))) {
            # For PPHG sensor, offset arrow slightly to the left
            pphg_labels <- subset_annotations[subset_annotations$Sensor == "PPHG", ]
            if(nrow(pphg_labels) > 0) {
            p <- p + 
                geom_segment(
                data = pphg_labels,
                aes(x = p_wave_time - 0.02, y = q_peak_amp, 
                    xend = p_wave_time - 0.02, yend = p_wave_amp),
                color = "#0066CC",
            ...
            # For AgCl sensor, offset arrow slightly to the right
        ...
        if (all(!is.na(subset_annotations$peak_to_peak_feature))) {
            # Add dotted reference lines for global minimum (one for each sensor, slightly offset)
            pphg_labels <- subset_annotations[subset_annotations$Sensor == "PPHG", ]
            if(nrow(pphg_labels) > 0) {
            p <- p + 
                geom_segment(
                data = pphg_labels,
                aes(x = r_peak_time - 0.1, y = global_min_amp, 
                ...
                # Arrow for PPHG shifted left
                geom_segment(
                data = pphg_labels,
                aes(x = r_peak_time - 0.02, y = global_min_amp, 
                    xend = r_peak_time - 0.02, yend = r_peak_amp),
                color = "#0066CC",
                arrow = arrow(type = "closed", length = unit(0.08, "inches")),
                linetype = "solid",
                size = 0.7,
            ...
            p <- p + 
                ...
                aes(x = r_peak_time, y = global_min_amp, 
                    xend = r_peak_time + 0.1, yend = global_min_amp),
                color = "#d9020d",
                linetype = "dotted",
                size = 0.8,
                inherit.aes = FALSE
                ) +
                # Arrow for AgCl shifted right
                geom_segment(
                data = agcl_labels,
                aes(x = r_peak_time + 0.02, y = global_min_amp, 
                    xend = r_peak_time + 0.02, yend = r_peak_amp),
        ...
        if (all(!is.na(subset_annotations$t_wave_feature))) {
            # For PPHG sensor, offset arrow to the left
            pphg_labels <- subset_annotations[subset_annotations$Sensor == "PPHG", ]
            if(nrow(pphg_labels) > 0) {
            p <- p + 
                geom_segment(
                data = pphg_labels,
                aes(x = t_wave_time - 0.02, y = q_peak_amp, 
                    xend = t_wave_time - 0.02, yend = t_wave_amp),
                color = "#0066CC",
                arrow = arrow(type = "closed", length = unit(0.08, "inches")),
                linetype = "solid",
                size = 0.7,
            ...
            # For AgCl sensor, offset arrow to the right
            agcl_labels <- subset_annotations[subset_annotations$Sensor == "AgCl", ]
            if(nrow(agcl_labels) > 0) {
            p <- p + 
                geom_segment(
      ...
      plot_list[[paste(participant, channel, sep = "_")]] <- p
    }
  }
}
...
if (length(plot_list) > 0) {
  ...
  ggsave("R_figures/figure_1d/ecg_kmeans_with_features.png", combined_plot, 
         width = 15, height = 10 * length(selected_participants) / 2, dpi = 300, bg = "white")
}

# Also create high-quality individual plots
for (plot_name in names(plot_list)) {
  ggsave(paste0("R_figures/figure_1d/", plot_name, "_features.png"), 
